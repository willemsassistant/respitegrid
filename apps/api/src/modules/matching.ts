...
export function computeSkillScore(
  ...
  const requiredMatches = required.filter((skill) =>
    caregiverSkills.has(skill),
  ).length;
  const preferredMatches = preferred.filter((skill) =>
    caregiverSkills.has(skill),
  ).length;
  const verifiedMatches = required.filter((skill) =>
    verifiedSkills.has(skill),
  ).length;

  const requiredRatio =
    required.length === 0 ? 1 : requiredMatches / required.length;
  const preferredRatio =
    preferred.length === 0 ? 0 : preferredMatches / preferred.length;
  const verifiedBonus =
    required.length === 0 ? 0 : verifiedMatches / required.length;

  return clamp(
    70 * requiredRatio + 20 * preferredRatio + 10 * verifiedBonus,
    0,
    100,
  );
}
...
export function computeAvailabilityScore(
  ...
  const hoursUntilVisit =
    (new Date(request.requestedStartAt).getTime() - now.getTime()) / 3_600_000;
  const shortNoticePenalty = hoursUntilVisit < 12 ? 10 : 0;
  const responseSpeedBonus =
    caregiver.medianOfferResponseMinutes <= 15
      ? 10
      : caregiver.medianOfferResponseMinutes <= 60
        ? 5
        : 0;
  const offerFatiguePenalty =
    caregiver.openOffersLast24h >= 5
      ? 10
      : caregiver.openOffersLast24h >= 3
        ? 5
        : 0;
  return clamp(
    90 + responseSpeedBonus - shortNoticePenalty - offerFatiguePenalty,
    0,
    100,
  );
}
...
export function computeReliabilityScore(caregiver: CaregiverProfile): number {
  ...
  const base =
    caregiver.completedVisitsCount < 3 ? 60 : caregiver.averageRating * 20;
...
export function computePreferenceScore(
  ...
  if (
    ...
    caregiver.languages.includes(recipient.preferredLanguage)
  )
    score += 20;
  if (
    recipient.petPresent &&
    caregiver.skills.some((skill) => skill.name === 'pet_friendly')
  )
    score += 10;
  if (
    (request.transportationRequired ||
      request.serviceType === 'transportation') &&
    caregiver.skills.some((skill) => skill.name === 'transportation')
  )
    score += 10;
...
export function computeContinuityScore(
  ...
  const ratingBonus =
    continuity.averageRating >= 4.8
      ? 30
      : continuity.averageRating >= 4.5
        ? 20
        : continuity.averageRating >= 4.0
          ? 10
          : 0;
...
export function computeRiskPenalty(
  ...
  if (recipient.fallRisk && !caregiverSkillNames.includes('fall_risk_support'))
    penalty += 25;
  if (
    recipient.cognitiveStatus === 'dementia' &&
    !caregiverSkillNames.includes('dementia_experience')
  )
    penalty += 30;
  if (
    (request.serviceType === 'transportation' ||
      request.transportationRequired) &&
    !caregiverSkillNames.includes('transportation')
  )
    penalty += 100;
  if (caregiver.completedVisitsCount < 3 && request.riskLevel === 'medium')
    penalty += 15;
  if (request.riskLevel === 'high' || request.riskLevel === 'blocked')
    penalty += 100;
...
export function computeFairnessAdjustment(caregiver: CaregiverProfile): number {
  ...
  if (
    caregiver.completedVisitsCount < 3 &&
...
export function buildMatchExplanation(input: {
  ...
  if (
    input.recipient.cognitiveStatus === 'dementia' &&
    input.caregiver.skills.some((skill) => skill.name === 'dementia_experience')
  ) {
    positive.push('Has dementia experience');
  }
  if (input.caregiver.averageRating >= 4.8)
    positive.push(
      `Average family rating ${input.caregiver.averageRating.toFixed(1)}`,
    );
  if (input.caregiver.completedVisitsCount > 0)
    positive.push(`Completed ${input.caregiver.completedVisitsCount} visits`);
...
export function computeCareFitScore(input: MatchInput): MatchScore {
  ...
  const availabilityScore = computeAvailabilityScore(
    input.request,
    input.caregiver,
    input.availabilityCovered ?? true,
  );
